...
## By chingliu
backend 依赖环境变量目前是设了默认值直接在\backend\app\core\config.py, .vscode有指定.env文件调试没问题
psycopg 导入如果有问题，可能是C:\Program Files\PostgreSQL\16\bin没加到环境变量PATH中去
初次运行要建立名为app的数据库，然后执行prestart.sh中的内容。
windows下可分别：
poetry.exe run python app/backend_pre_start.py
poetry.exe run alembic upgrade head
poetry.exe run python app/initial_data.py
